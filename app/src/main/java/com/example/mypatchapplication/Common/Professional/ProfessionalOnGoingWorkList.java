...
import com.example.mypatchapplication.Helperclass.ProfessionalOnGoingRVAdapter;
...
public class ProfessionalOnGoingWorkList extends AppCompatActivity {
    ...
    DatabaseReference allonGoingRef;
    private List<HireUsModel> hireUsModelList = new ArrayList<>();

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_professional_on_going_work_list);
        ...
        historyRecycler = findViewById(R.id.recycler_ongoing_list);
        progressHistory = findViewById(R.id.user_ongoinglist_progressbar);

        progressHistory.setVisibility(View.VISIBLE);
        allonGoingRef = database.getReference("HireList");
        allonGoingRef.addChildEventListener(new ChildEventListener() {
            @Override
                ...
                if (dataSnapshot.exists()) {
                    ...
                    for (DataSnapshot datas : dataSnapshot.getChildren()) {
                        ...
                        if(datas.child("professionalID").getValue().toString().equals(current_ID) && datas.child("responsestatus").getValue().toString().equals("adminresponse")) {
                            ...
                            ProfessionalOnGoingRVAdapter professionalOnGoingRVAdapter  = new ProfessionalOnGoingRVAdapter(hireUsModelList, ProfessionalOnGoingWorkList.this);
                            RecyclerView.LayoutManager layoutManager = new LinearLayoutManager(ProfessionalOnGoingWorkList.this);
                            historyRecycler.setLayoutManager(layoutManager);
                            historyRecycler.setHasFixedSize(true);
                            historyRecycler.setAdapter(professionalOnGoingRVAdapter);
                        }
                    }
                }
            }
    ...
    public void onGoingList_goback(View view) {
        ProfessionalOnGoingWorkList.super.onBackPressed();
    }
